...
public class WechatPay extends Pay {
	...
	WechatPay(int payType, IPayListener listener) {
	...
	protected void payBefore(Activity activity) {
		TAGS.log("--------------------------微信支付------------------------------");
		payBeforeSendMessage(PAY_TYPE_WEIXIN);
	}
	...
	protected Result payDoing(Activity activity) {
		...
		String tmpResult = HttpUtils.sendPostUTF8(getHost() + "/pay/unifiedorder", mParamMap);
		...
		try {
			...
			if("success".equalsIgnoreCase(code)) {
				...
				startPayH5Activity(activity, mweb_url, DYH5Activity.PAGE_WEIXIN, DYH5Activity.GET);
	...
	protected boolean isLaunchPay(Activity activity) {
		...
		if(!isLaunchPay)
			Toast.makeText(activity, "支付前请先安装微信客户端", Toast.LENGTH_SHORT).show();
		return isLaunchPay;
	}
